...
export default function RangeSlider({
  ...
  const minBoundary = Number(boundaryMinP);
  const maxBoundary = Number(boundaryMaxP);
  const min_initVal = initValMinP;
  const max_initVal = initValMaxP;
  ...
  const min_getPanResponder = () => {
    return PanResponder.create({
      ...
      onPanResponderRelease: (
        ...
        // setMinDistance(min_animState.currentVal);
        // setMinPrice(min_animState.currentVal);
  ...
  const [max_initOffset, max_setInitOffset] = React.useState(-10);
  ...
  const max_getPanResponder = () => {
    return PanResponder.create({
      ...
      onPanResponderRelease: (evt, gestureState) => {
        ...
        // setMaxPrice(max_animState.currentVal);
        // setMaxDistance(max_animState.currentVal);
  ...
  useEffect(() => {
    setMinDistance(min_animState.displayVal);
    setMaxDistance(max_animState.displayVal);
  }, [min_animState.displayVal, max_animState.displayVal]);

  // setMinDistance(min_animState.displayVal);
  // setMaxDistance(max_animState.displayVal);
  // setMinPrice(min_animState.displayVal);
  // setMaxPrice(max_animState.displayVal);
  // ----------------- Render ----------------------- //
  return (
      ...
      <View style={s.container}>
        <View style={s.labelValue}>
          <Text style={s.labelValueText}>
            {prefix} {min_animState.displayVal} {suffix}
          </Text>
        </View>
        <View
          style={[
